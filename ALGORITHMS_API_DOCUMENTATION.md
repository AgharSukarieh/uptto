...
1. **`src/Service/algorithmService.js`**:
   - `getAllAlgorithmsWithTags()` - جلب جميع الخوارزميات مع الفئات
   - `getAlgorithmById(algorithmId)` - جلب تفاصيل خوارزمية محددة

2. **`src/Pages/Algorithms/Algorithms.js`**:
   - صفحة عرض جميع الفئات والخوارزميات
   - استخدام `getAllAlgorithmsWithTags()` لجلب البيانات

3. **`src/Pages/Algorithms/AlgorithmDetail.js`**:
   - صفحة تفاصيل الخوارزمية
   - استخدام `getAlgorithmById()` لجلب التفاصيل

### **ميزات التطبيق:**

✅ معالجة تلقائية للـ Token  
✅ تجديد Token تلقائياً عند انتهاء الصلاحية  
✅ التحقق من صحة البيانات (arrays validation)  
✅ معالجة الأخطاء بشكل شامل  
✅ Console logs للمساعدة في debugging  
